...
import {Form,Input,Button,Row,Col,Icon,Select,Table,message} from 'antd';
...
class VipPoint extends React.Component{
    constructor(props){
        super(props);
    ...
    fetchData(page){
        let actionInfo=  {"action":"getUserIntergralLists","page":page,"limit":this.state.pageSize,"user_id":this.props.params.user_id,"order_number":this.state.ordernum,"begin_time":this.state.startValue,"end_time":this.state.endValue};
        SettingModel.fetchData(actionInfo,_success=>{
            if(_success.status && this._isMounted){
                let _list=[];
                _success.result.integralRows.map((item,index)=>{
                    var it=item;
                    it.key=index+1;
                    _list.push(it);
                });
