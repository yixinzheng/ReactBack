...
class NewsSort extends React.Component{
    constructor(props){
        super(props);
        this.state={
            title:'',
        ...
        this.props.form.validateFields((err, values) => {
            if(!err){
                ...
                if(this.state.type=='Add'){
                    let _p={"action":"addNewsCategory","title":values.sortname};
                    ...
                    let _p= {"action":"editNewsCategory","newscategory_id":this.props.params.id,"title":values.sortname};
                    SettingModel.fetchData(_p,_succ=>{
                        if(_succ.status){
                            message.success('操作成功');
                            history.go(-1);
