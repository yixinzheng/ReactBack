...
class HelpListAdd extends React.Component{
    constructor(props){
        super(props);
    ...
    fetchAllData(){
        let _p={"action":"getHelpDetailByHelpId","helpnews_id":this.state.id};
        SettingModel.fetchData(_p,_succ=>{
            if(_succ.status && this._isMounted){
                this.props.form.setFieldsValue({
                    title:_succ.result.title,
                    category:_succ.result.helpcategory_id
                });
                this.setState({
    ...
    fetchData(){
        let _p={"action":"getHelpCategoryListsNoPage"};
        SettingModel.fetchData(_p,_succ=>{
            if(_succ.status && this._isMounted){
                this.setState({
    ...
    handleSubmit(e){
        ...
        this.props.form.validateFields((err, values) => {
            ...
            let _p=action=="Add"?({"action":"addHelp","title":values.title,"helpcategory_id":values.category,"content":this.refs.editor.getContent()}):({"action":"editHelp","helpnews_id":this.state.id,"title":values.title,"helpcategory_id":values.category,"content":this.refs.editor.getContent()});
            SettingModel.fetchData(_p,_succ=>{
                if(_succ.status){
            ...
            <div>
                <Row className="addsort-title">
                    <Col span={2} className="txt-center">
                        {this.state.action=="Modify"?"修改帮助文章":"添加帮助文章"}
                    </Col>
                </Row>
...
HelpListAdd=Form.create()(HelpListAdd);
export default HelpListAdd;