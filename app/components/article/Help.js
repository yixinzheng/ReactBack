...
class Help extends React.Component{
    ...
    fetchData(page){
        let _p={"action":"getHelpCategoryLists","page":page,"limit":this.state.pageSize,"title":this.state.sname};
    ...
    columnTb(){
        return [{
            ...
            render:(text, record) => (
                <div>
                    <Link to={`/article/help/sortEdit/id=${record.helpcategory_id}&name=${record.title}`}>修改</Link>
                    <span className="ant-divider" />
                    <span> <Link to={`/article/help/helpList${record.helpcategory_id}`}>查看</Link></span>

                    {parseInt(record.count)>0 ? '' : <span><span className="ant-divider" /><Popconfirm title="确定要删除该数据吗?" onConfirm={()=>this.confirm(record.helpcategory_id)} onCancel={this.cancel} okText="确认" cancelText="取消">
                        <a href="#">删除</a>
                    </Popconfirm></span> }
                </div>
            )
    ...
    confirm(ind) {
        if(ind!=''){
            let _p={"action":"editHelpCategory","helpcategory_id":ind,"status":"2"};
            SettingModel.fetchData(_p,_success=>{
                if(_success.status){
    ...
    render(){
        ...
        return (
            <div className="setting-ad">
                <SearchName searchData={(info)=>this.searchFetch(info)} link={`/article/help/sortAdd`} ntitle="分类名称" btitle="新增分类"/>
