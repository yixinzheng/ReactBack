...
class HelpSort extends React.Component{
    constructor(props){
        super(props);
        this.state={
            title:'',
        ...
        this.props.form.validateFields((err, values) => {
            if(!err){
                ...
                if(this.state.type=='Add'){
                    let _p={"action":"addHelpCategory","title":values.sortname};
                    ...
                    let _p={"action":"editHelpCategory","helpcategory_id":this.props.params.id,"title":values.sortname};
                    SettingModel.fetchData(_p,_succ=>{
                        if(_succ.status){
                            message.success('操作成功');
                            history.go(-1);
...
HelpSort=Form.create()(HelpSort);
export default HelpSort;