...
class HelpList extends React.Component{
    ...
    fetchData(page){
        ...
        let _p={"action":"getHelpListByCateId","title":this.state.sname,"page":page,"limit":this.state.pageSize,"helpcategory_id":id};
        SettingModel.fetchData(_p,_success=>{
            if(_success.status && this._isMounted){
                let _list=[];
                _success.result.helpRows.map((item,index)=>{
                    var it=item;
                    it.key=index+1;
                    _list.push(it);
                });
    ...
    confirm(ind) {
        if(ind!=''){
            let _p= {"action":"editHelp","helpnews_id":ind,"status":"2"};
            SettingModel.fetchData(_p,_success=>{
                if(_success.status){
    ...
    blurOrder(e,record){
        ...
        if(change_value<=0 && change_value!=""){
            ...
            let _p= {"action":"editHelp","helpnews_id":record.helpnews_id,"order_number":change_value};
    ...
    columnTb(){
        return [{
            ...
            render:(text, record) => (
                <div>
                    <Link to={`/article/help/articleModify/${record.helpnews_id}`}>修改</Link>
                    <span className="ant-divider" />
                    <Popconfirm title="确定要删除该数据吗?" onConfirm={()=>this.confirm(record.helpnews_id)} onCancel={this.cancel} okText="确认" cancelText="取消">
                        <a href="#">删除</a>
                    </Popconfirm>
                </div>
            )
    ...
    render(){
        ...
        return (
            <div className="setting-ad">
                <SearchName searchData={(info)=>this.searchFetch(info)} link={`/article/help/articleAdd/${this.state.id}`} ntitle="文章标题" btitle="新增文章"/>
